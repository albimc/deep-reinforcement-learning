...
def create_tiling_grid(low, high, bins=(10, 10), offsets=(0.0, 0.0)):
    ...
    """
    grid = [offsets[i] + np.linspace(low[i], high[i], bins[i]+1)[1:-1] for i in range(len(low))]
...
def visualize_tilings(tilings):
    ...
    for i, grid in enumerate(tilings):
        for x in grid[0]:
            ll = ax.axvline(x=x, color=colors[i % len(colors)], linestyle=linestyles[i % len(linestyles)], label=i)
        for y in grid[1]:
            ll = ax.axhline(y=y, color=colors[i % len(colors)], linestyle=linestyles[i % len(linestyles)])
